...
const BASE32_DECODE_CHAR: {[k: string]: number} = {
...
const throwInvalidUtf8 = function (position: number, partial:string) {
    ...
    var err: Error & {position?: any} = new Error('Decoded data is not valid UTF-8.'
...
const toUtf8String = function (bytes: number[]) {
    let str = '', length = bytes.length, i = 0, followingChars = 0, b, c;
    while (i < length) {
        b = bytes[i++];
        if (b <= 0x7F) {
...
const decodeAsBytes = function (base32Str: string): number[] {
    if (base32Str === '') {
        return [];
...
const encodeAscii = function (str: string) {
    let v1, v2, v3, v4, v5, base32Str = '', length = str.length;
    for (var i = 0, count = parseInt((length / 5) as unknown as string) * 5; i < count;) {
        v1 = str.charCodeAt(i++);
        v2 = str.charCodeAt(i++);
        v3 = str.charCodeAt(i++);
        v4 = str.charCodeAt(i++);
...
export const encode = function (input: string | ArrayBuffer | Uint8Array | number[], asciiOnly?: boolean) {
...
export const decode = function (base32Str: string, asciiOnly?: boolean) {
    ...
    let v1, v2, v3, v4, v5, v6, v7, v8, str = '', length = base32Str.indexOf('=');
    if (length === -1) {
